...
class FireflyAlgorithm():
        ...
        for i in range(self.NP):
            for j in range(self.D):
                self.Fireflies[i][j] = random.uniform(
                    0, 1) * (self.UB[j] - self.LB[j]) + self.LB[j]  # changed
    ...
    def FindLimits(self, k):
        for i in range(self.D):
            if self.Fireflies[k][i] < self.LB[i]:
                self.Fireflies[k][i] = self.LB[i]  # changed
            if self.Fireflies[k][i] > self.UB[i]:
                self.Fireflies[k][i] = self.UB[i]

    def move_ffa(self):
        for i in range(self.NP):
            scale = abs(max(self.UB) - min(self.LB))  # changed
            for j in range(self.NP):
                ...
                if self.I[i] > self.I[j]:  # brighter and more attractive
                    ...
                    for k in range(self.D):
                        ...
                        self.Fireflies[i][k] = self.Fireflies[i][k] * \
                                               (1.0 - beta) + self.Fireflies_tmp[j][k] * beta + tmpf
            self.FindLimits(i)
    ...
    def Run(self):
        ...
        return self.Fireflies[0], self.fbest, {end-start}
